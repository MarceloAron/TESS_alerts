...
'P', 'T0', 'Rp', 'a', 'inc']
csvfile = open('spoctable.csv','w')
writer = csv.writer(csvfile,delimiter=',',quotechar='|',quoting=csv.QUOTE_MINIMAL)
writer.writerow(header) 

with PdfPages('spoc.pdf') as pdf:
	### Open .fits file
	fname = 'fits/*s_llc.fits'
...
with PdfPages('test.pdf') as pdf:
	...
	pipe = 'spoc'
	### Open .fits file
	fname = 'fits/*{}*'.format(TIC) 
	fits = glob.glob(fname)[0]
	file = pyfits.open(fits)
	hdr = file[0].header
	data = file[1].data
	...
	if pipe == 'spoc':
		...
		pp = np.polyfit(bjd,flux,2)
		flux = flux/(pp[0]*bjd**2 + pp[1]*bjd + pp[2])
		errflux = flux*relerr
	...
	BLS,results = boxleastsq.boxleastsq(bjd,flux,errflux)
	...
	mid = np.argmin(np.sqrt((bjd%per - fold)**2))
	T0 = float(bjd[mid]) # actual midtransit time
	...
	### Get best fitting model
	lc_fit = fit.fit_LC(bjd, flux, errflux, P=per, T0=T0, a=a, Rp=Rp, inc=90., ecc=0., omega=90., LD=limb, c1=c1, c2=c2)
	print(lmfit.fit_report(lc_fit, show_correl=False))
	per = lc_fit.params['P'].value
	T0 = lc_fit.params['T0'].value
	a = lc_fit.params['a'].value
	...
	ax = fig.add_subplot(1,1,1)
	ax.set_title(r'$\rm Sector \ {:d}$'.format(SEC))
	ax.spines['top'].set_color('none')
	ax.spines['bottom'].set_color('none')
	ax.spines['left'].set_color('none')
	ax.spines['right'].set_color('none')
	...
	r'$P = {:.4f} \ \rm days$'.format(BLS['period']), 
	r'$t_{}={:0.4f} \ \rm days$'.format('\mathrm{T}',tT),
	r'$\delta={:0.2f}$'.format(depth),
	r'$\delta_{}={:0.2f}$'.format('\mathrm{odd}',dep_odd),
	r'$\delta_{}={:0.2f}$'.format('\mathrm{even}',dep_even),
	r'$\rm Binary: \ {}$'.format(binary)]
	
	font = 8
	for idx, val in enumerate(left): axt.text(-4,idx,val,fontsize=font)
	
	right = [r'$T_{} = {} \ \rm K$'.format('\mathrm{eff}',Teff_L),
	r'$\log g = {} \ \rm dex$'.format(logg_L), 
	r'$\rm [M/H] = {} \ \rm dex$'.format(MeH_L),
	r'$\ $',
	r'$\rm {\bf LMfit}$',
	...
	r'$R_{} = {:.3f} \ R_\star$'.format('\mathrm{p}',Rp),
	r'$a = {:.3f} \ R_\star$'.format(a),
	r'$i = {:.0f} ^\circ$'.format(inc), 
	r'$e = {:.2f}$'.format(ecc)]
	
	for ll, val in enumerate(right): axt.text(1,ll,val,fontsize=font)
	
	axt.set_xlim(-4,4)
	axt.set_ylim(len(right)-1,-1)

	### Plot light curve
	ax1 = fig.add_subplot(2,2,1)
	ax1.plot(phase,flux,'.',markersize=1.0,color=cols[7])
	ax1.plot(phase[shift],lc[shift],'-',color=cols[0],linewidth=3)
	dur_h = (tT*24)/2
	ax1.set_xlim(-1*(dur_h+2),dur_h+2)
	ax1.set_xlabel(r'$\rm Hours \ from \ Midtransit$')
	ax1.set_ylabel(r'$\rm Relative \ Brightness$')
	...
	ax2.axvline(per,color=cols[0],lw=3)
	ax2.plot(results.period,results.power,'k',lw=0.5)
	ax2.set_xlabel(r'$\rm Period \ (days)$')
	ax2.set_ylabel(r'$\rm \log \ likelihood$')
